...
public:
  ...
  void assign(size_type count, const_reference value) {
    ...
    if (count > _size) {
      ...
      ft::uninitialized_fill_n_a(_start + _size, diff, value, _alloc);
      _size += diff;
      return;
    }
  ...
  assign(InputIt first, InputIt last) {
    size_type new_cap = static_cast<size_type>(ft::distance(first, last));
    this->reserve(new_cap);

    size_type i = 0;
    while (i < _size && first != last) {
      _start[i] = *first;
      ++i; ++first;
    }

    ft::uninitialized_copy_a(first, last, _start + i, _alloc);
    while (i < _size) {
      _alloc.destroy(_start + i);
      ++i;
    }
  ...
  insert(iterator pos, InputIt first, InputIt last) {
    ...
    try
    { ft::uninitialized_copy_a(pos, this->end(), tmp + _n + count, _alloc); }
    catch (...) {
  ...
  void pop_back(void) { --_size; _alloc.destroy(_start + _size); }
